  ...
  const handleApply = async () => {
    ...
    try {
      ...
      const response = await applicationAPI.submitApplication({
        ...
        resumeUrl: 'https://example.com/my-resume.pdf' // 这里可以改为实际的简历URL
      });
      ...
      // 处理特定错误
      if (error.response?.data?.error?.code === 'DUPLICATE_ENTRY') {
        alert('You have already applied to this job!');
        setIsApplied(true);
      } else if (error.response?.data?.error?.code === 'DEADLINE_PASSED') {
        alert('Sorry, the application deadline has passed for this job.');
      } else {
        alert(error.response?.data?.error?.message || 'Failed to submit application. Please try again.');
      }
    } finally {
      setIsApplying(false);
    }
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          ...
          <button 
            ...
            style={{
              ...
              borderRadius: '6px',
              cursor: (isApplied || isApplying || (daysLeft !== null && daysLeft < 0)) ? 'not-allowed' : 'pointer',
              fontSize: '1rem',
              fontWeight: '600',
              transition: 'all 0.3s ease',
            ...
            onMouseEnter={(e) => {
              if (!isApplied && !isApplying && (daysLeft === null || daysLeft >= 0)) {
